...
if (IsSet($_GET["hlavolam"])){
    ...
    if (!is_numeric($hlavolam)){  
        $vypis=mysql_query("select * from hlavolamy where link = '$hlavolam'") or die(mysql_error());
        }
    else{
        $vypis=mysql_query("select * from hlavolamy where id = '$hlavolam'") or die(mysql_error());
        }
    ...
    $vypis=mysql_query("select * from hlavolamy  where zobrazit = '1' and kat = '0' order by id DESC");
    echo "<br />";
