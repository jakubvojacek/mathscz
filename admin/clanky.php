...
if (!IsSet($_GET["zobraz"])){
    ...
    $vypis = mysqli_query(DATABASE::getDb(), "select upraven, link, autor, jmeno, id from clanky where dokoncen = 'ne' $add order by id desc"); 

    ukaz_clanky("Nevydané články", $vypis);

    $vypis = mysqli_query(DATABASE::getDb(), "select upraven, link, autor, jmeno, id from clanky where dokoncen = 'ano' $add order by id desc"); 
