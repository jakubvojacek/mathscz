...
function get_cat($kat){
    $vypis = mysqli_query(DATABASE::getDb(), "select * from kategorie where link = '$kat'");
...
if ($nadrazena != -1){
    $vypis = mysqli_fetch_array(mysqli_query(DATABASE::getDb(), "select * from kategorie where id = '$nadrazena'"));  
...
$vypis = mysqli_query(DATABASE::getDb(), "select * from kategorie where nadrazena = '$kategorie'");
$pocet_pod = mysqli_num_rows($vypis);
$k = ceil($pocet_pod / 3);
...
function vrat_podrazene($id, $pole_kategorii){    
    $vypis = mysqli_query(DATABASE::getDb(), "select * from kategorie where nadrazena = '$id'");
    while($vysledek = mysqli_fetch_array($vypis)){
...
$vypis = mysqli_query(DATABASE::getDb(), "select * from clanky where dokoncen = 'ano' and (kategorie = '$kopie_kategorie' $pole_kategorii)") or die(mysql_error());
$pocet_pod = mysqli_num_rows($vypis);
$zobrazit_reklamu = 1;
if ($kategorie == "10"){
    $zobrazit_reklamu = 0;
